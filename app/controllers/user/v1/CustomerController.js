...
module.exports = new (class CustomerController extends Controller {
  ...
  async getCustomers(req, res) {
    try {
      ...
      req
        ...
        .isInt({ min: 0 });
      req
        .checkParams("orderStatus", "please enter order status")
        .notEmpty()
        .isInt({ min: 0, max: 2 }); //0 -> fail, 1 -> success, 2 -> undefined
      ...
      if (req.params.orderStatus === "0") filter.failOrders = { $gt: 0 };
  ...
  async getCustomer(req, res) {
    try {
      ...
      let seller = await this.model.Seller.find(filter, {
        family: 1,
        mobile: 1,
      })
        .sort({ createdAT: -1 })
      ...
      if (!seller.length && !customer.length)
        return res.json({
          success: true,
      ...
      return res.json({
        ...
        data: { customer: customer[0] || false, seller: seller[0] || false },
      });
    } catch (err) {
      let handelError = new this.transforms.ErrorTransform(err)
        .parent(this.controllerTag)
